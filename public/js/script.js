...
const botaoAbrirAgendamento = document.getElementById("botao-abrir-agendamento");
...
const observacoesListaEspera = document.getElementById("observacoes-lista-espera");
const botaoEntrarListaEspera = document.getElementById("botao-entrar-lista-espera");
...
const painelSucessoAgendamento = document.getElementById("painel-sucesso-agendamento");
const textoSucessoAgendamento = document.getElementById("texto-sucesso-agendamento");
const linkWhatsappConfirmacao = document.getElementById("link-whatsapp-confirmacao");
const botaoAdicionarCalendario = document.getElementById("botao-adicionar-calendario");
...
const botaoToggleAreaCliente = document.getElementById("botao-toggle-area-cliente");
...
const listaAgendamentosCliente = document.getElementById("lista-agendamentos-cliente");
const painelHistoricoCliente = document.getElementById("painel-historico-cliente");
const listaHistoricoCliente = document.getElementById("lista-historico-cliente");
const elementosModoSimplesOcultos = document.querySelectorAll('[data-modo-simples="ocultar"]');
const elementosModoSimplesAlternaveis = document.querySelectorAll('[data-modo-simples="alternar"]');
...
function renderizarHorarios() {
    gradeHorarios.innerHTML = HORARIOS_PADRAO.map((horario) => `
...
function atualizarResumo() {
    ...
    resumoServico.textContent = servicoSelecionado
        ...
        : "-";
    resumoData.textContent = dataSelecionada ? formatarData(dataSelecionada) : "-";
    ...
    secaoConfirmacao.classList.toggle("oculto", !podeMostrarConfirmacao && !painelSucessoAtivo);
}
...
function atualizarTextoDataSelecionada() {
    if (!dataSelecionada) {
        textoDataSelecionada.textContent = "Semana atual visivel abaixo. Para outra data, use o botao Escolher data.";
        return;
    }
...
function formatarStatusHistorico(agendamento) {
    ...
    return new Date(dataHoraAgendamento) >= new Date() ? "Agendado" : "Finalizado";
}
...
function obterDuracaoEmMinutos(textoDuracao) {
    const valor = String(textoDuracao || "").toLowerCase().trim();
...
function criarConteudoCalendario(agendamento) {
    ...
    const inicio = new Date(`${agendamento.dataAgendamento}T${agendamento.horarioAgendamento}:00`);
    ...
    const inicioTexto = formatarDataParaIcs(agendamento.dataAgendamento, agendamento.horarioAgendamento);
...
async function carregarDatasOcupadas() {
    ...
    try {
        ...
        const resposta = await fetch(
            obterUrlApi(`/api/datas-ocupadas?inicio=${encodeURIComponent(periodo.inicio)}&dias=${periodo.totalDias}`)
        );
        const tipoConteudo = resposta.headers.get("content-type") || "";

        if (!tipoConteudo.includes("application/json")) {
            throw new Error("A API nao respondeu em JSON. Rode o Node em http://127.0.0.1:3000.");
        }

        const dados = await resposta.json();

        if (!resposta.ok) {
            throw new Error(dados.erro || "Nao foi possivel consultar as datas ocupadas.");
        }

        datasComOcupacao = new Map((dados.datas || []).map((item) => [item.data, item]));
        renderizarDatas();
        textoAjudaDatas.textContent = "A selecao rapida mostra apenas a semana atual. Para outras datas, use o calendario.";
    } catch (erro) {
...
function prepararHorariosSemData() {
    ...
    textoAjudaHorarios.textContent = "Escolha uma data para consultar os horarios.";
    ocultarListaEspera();
    atualizarResumo();
}
...
function selecionarHorario(botao) {
    ...
    if (nome.length < 3 || telefone.length < 8) {
        window.alert("Por favor, preencha o seu nome e telefone antes de escolher o horário.");
        rolarParaElemento(campoNome.closest(".grade-campos") || campoNome, campoNome);
        return;
    }
...
async function carregarHorariosDaData(data) {
    ...
    try {
        const resposta = await fetch(obterUrlApi(`/api/horarios?data=${encodeURIComponent(data)}`));
        const tipoConteudo = resposta.headers.get("content-type") || "";
        ...
        if (dados.diaFechado) {
            ...
            textoAjudaHorarios.textContent = "Domingo e folga. Escolha um dia de segunda a sabado.";
            return;
        }

        obterBotoesHorario().forEach((botao) => {
            const horario = botao.dataset.horario;
            const horarioIndisponivel = (dados.horariosIndisponiveis || dados.horariosOcupados || []).includes(horario);
        ...
        if (dados.horariosDisponiveis.length === 0) {
            ...
            textoAjudaHorarios.textContent = "Alguns horarios foram bloqueados pela barbearia. Escolha uma opcao livre.";
        } else if ((dados.horariosPassados || []).length > 0 && data === campoData.min) {
            textoAjudaHorarios.textContent = "Horarios que ja passaram hoje foram riscados. Escolha um horario futuro.";
        } else {
            textoAjudaHorarios.textContent = "Horarios livres carregados. Escolha um horario e finalize em segundos.";
        }
...
function restaurarCredenciaisCliente() {
    try {
        const clienteSalvo = JSON.parse(window.localStorage.getItem(CHAVE_CLIENTE) || "null");
...
function renderizarListaAgendamentosCliente(agendamentos) {
    ...
    painelAgendamentosCliente.classList.remove("oculto");
    listaAgendamentosCliente.innerHTML = agendamentos.map((agendamento) => `
...
function renderizarHistoricoCliente(historico) {
    ...
    listaHistoricoCliente.innerHTML = historico.map((agendamento) => `
        <article class="item-agendamento-cliente">
            <div class="item-agendamento-cliente-topo">
                <strong>${agendamento.servico}</strong>
...
async function carregarAgendamentosCliente() {
    ...
    try {
        ...
        const resposta = await fetch(obterUrlApi(`/api/meus-agendamentos?${parametros.toString()}`));
        const tipoConteudo = resposta.headers.get("content-type") || "";
        ...
        if (!resposta.ok) {
            throw new Error(dados.erro || "Nao foi possivel localizar seus agendamentos.");
        }

        renderizarListaAgendamentosCliente(dados.agendamentos || []);
        mostrarMensagemLogin("Acesso liberado. Voce pode cancelar seu proprio agendamento por aqui.", "sucesso");
    } catch (erro) {
        painelAgendamentosCliente.classList.add("oculto");
        listaAgendamentosCliente.innerHTML = "";
...
async function carregarHistoricoCliente() {
    ...
    try {
        const parametros = new URLSearchParams(clienteLogado);
        const resposta = await fetch(obterUrlApi(`/api/historico-cliente?${parametros.toString()}`));
        const tipoConteudo = resposta.headers.get("content-type") || "";
        ...
        if (!resposta.ok) {
            throw new Error(dados.erro || "Nao foi possivel localizar seu historico.");
        }
...
async function entrarCliente() {
    ...
    if (credenciais.nomeCliente.length < 3 || normalizarTelefone(credenciais.telefoneCliente).length < 8) {
        mostrarMensagemLogin("Informe nome e telefone validos para localizar seus agendamentos.", "erro");
        return;
    }
...
async function cancelarAgendamentoCliente(idAgendamento, botao) {
    ...
    try {
        const resposta = await fetch(obterUrlApi(`/api/agendamentos/${idAgendamento}/cancelar`), {
            method: "PATCH",
            headers: {
...
async function salvarListaEspera() {
    if (!servicoSelecionado || !dataSelecionada) {
        mostrarMensagemListaEspera("Escolha um servico e uma data antes de entrar na lista de espera.", "erro");
        return;
    }
    ...
    if (nomeCliente.length < 3 || normalizarTelefone(telefoneCliente).length < 8) {
        mostrarMensagemListaEspera("Preencha nome e telefone validos para entrar na lista de espera.", "erro");
        return;
    }
    ...
    try {
        ...
        if (!resposta.ok) {
            throw new Error(dados.erro || "Nao foi possivel entrar na lista de espera.");
        }
...
function selecionarServicoPorNome(nomeServico) {
    const entrada = Array.from(entradasServico).find((item) => item.value === nomeServico);
...
botaoAbrirAgendamento.addEventListener("click", () => mostrarSecaoHorarios(true));
...
botaoAdicionarCalendario.addEventListener("click", () => {
    ...
    baixarArquivo("agendamento-barber-shop-custom.ics", conteudo, "text/calendar;charset=utf-8;");
});
...
if (botaoToggleAreaCliente) {
    botaoToggleAreaCliente.addEventListener("click", alternarAreaClienteModoSimples);
}
